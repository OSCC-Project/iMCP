...
def run_ieda(iEDA: Path, script_path: str, design_name: str):
    """Run iEDA with the given script path."""
    
    import subprocess  
    script = f"cd /home/taosimin/IR_test25/script && \
        source config_env.sh {design_name} && \
        {iEDA} -script {script_path}"
    
    logging.info(f"Run iEDA with script: {script}")

    process = subprocess.run(script, shell=True, executable="/bin/bash", check=True)
...
def serve(iEDA: Path, transport="stdio"):
    ...
    async def call_tool(tool: str, arguments: dict) -> list[TextContent]:
        if tool == iEDAMcpTools.iEDA_RUN:
            ...
            example_script_path = f"/home/taosimin/IR_test25/script/run_ir.tcl"
            if os.path.exists(example_script_path):
                run_ieda(iEDA, example_script_path, example_name)
